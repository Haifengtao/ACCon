...
# mine
parser.add_argument('--proj_dims', type=int, default=512, help='regularization_type')
...
parser.add_argument('--datatype', type=str, default='natural', choices=['natural', 'balanced'],
...
def main():
    ...
    if args.regularization_type != "ada":
        ...
        train_dataset = IMDBWIKI_ada(data_dir=args.data_dir, n_views=args.n_views, df=df_train, img_size=args.img_size,
                                     split='train',
                                     reweight=args.reweight, lds=args.lds, lds_kernel=args.lds_kernel, lds_ks=args.lds_ks,
                                     lds_sigma=args.lds_sigma)
        val_dataset = IMDBWIKI_ada(data_dir=args.data_dir, n_views=args.n_views, df=df_val, img_size=args.img_size,
                                   split='val')
        test_dataset = IMDBWIKI_ada(data_dir=args.data_dir, n_views=args.n_views, df=df_test, img_size=args.img_size,
                                    split='test')
...
def train(args, Con_Loss, train_loader, model, traindata_size, optimizer, epoch, accum_iter=1):
    ...
    for idx, batch in enumerate(train_loader):
        ...
        #     break
        if args.loss == 'ada':
            inputs, targets, targets_pdf, weights = batch
            targets, targets_pdf, weights = targets.cuda(non_blocking=True), targets_pdf.cuda(non_blocking=True), weights.cuda(non_blocking=True)
        else:
            inputs, targets, weights = batch
            targets, weights = targets.cuda(non_blocking=True), weights.cuda(non_blocking=True)
        data_time.update(time.time() - end)

        if args.model == "contraNet_ada_emb":
            bsz = targets.size()[0]
            ...
            if args.regularization_weight > 0:
                cons_loss = args.regularization_weight * \
                            Con_Loss(features, targets, dist=None, norm_val=2 / traindata_size, scale_s=20)
...
def validate(args, val_loader, model, train_labels=None, prefix='Val'):
    ...
    with torch.no_grad():
        ...
        for idx, batch in enumerate(val_loader):
            ...
            if args.loss == 'ada':
                inputs, targets, targets_pdf, weights = batch
                targets, targets_pdf, weights = targets.cuda(non_blocking=True), targets_pdf.cuda(
                    non_blocking=True), weights.cuda(non_blocking=True)
            else:
                inputs, targets, weights = batch
                targets, weights = targets.cuda(non_blocking=True), weights.cuda(non_blocking=True)
            inputs = inputs[0].cuda(non_blocking=True)
