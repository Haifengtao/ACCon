...
from tasks import STSBTask, STSBTask_Natural

PATH_PREFIX = './glue_data/'

ALL_TASKS = ['sts-b']
NAME2INFO = {'sts-b': (STSBTask, 'STS-B/'), 'sts-b-n': (STSBTask_Natural, 'STS-B/')}
...
def build_tasks(args):
    ...
    if vis:
        ...
        plt.hist(tasks[0].test_data_text[-1], np.array(range(50)) / 10, color="salmon", rwidth=0.75)
        plt.title(f"Test: {len(tasks[0].train_data_text[-1])}")
        plt.show()
    ...
    for task in tasks:
        train, val, test = process_task(task, token_indexer, vocab, args)
...
def process_task(task, token_indexer, vocab, args=None):
    ...
    if hasattr(task, 'train_data_text') and task.train_data_text is not None:
        train = process_split(task.train_data_text, token_indexer, args=args)
    else:
        train = None
    if hasattr(task, 'val_data_text') and task.val_data_text is not None:
        val = process_split(task.val_data_text, token_indexer, args=args)
    else:
        val = None
    if hasattr(task, 'test_data_text') and task.test_data_text is not None:
        test = process_split(task.test_data_text, token_indexer, args=args)
    else:
        test = None
...
def process_split(split, indexers, args=None):
    '''
    ...
    # pdb.set_trace()
    if len(split) == 5:
        weights = [NumericField(w) for w in split[3]]
        pdfs = [NumericField(w) for w in split[4]]
        instances = [Instance({"input1": input1, "input2": input2, "label": label, "weight": weight, "pdf": pdf}) for \
                     (input1, input2, label, weight, pdf) in zip(inputs1, inputs2, labels, weights, pdfs)]
    elif len(split) == 4:
        # import pdb
        # pdb.set_trace()
        if  args.regularization_type == "ada":  # weight
            weights = [NumericField(w) for w in split[3]]
            instances = [Instance({"input1": input1, "input2": input2, "label": label, "pdf": weight}) for \
