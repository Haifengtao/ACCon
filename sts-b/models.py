...
def build_model(args, vocab, pretrained_embs, tasks):
    ...
    build_projection(model, d_pair * 4, dims=d_pair)
    # import pdb
    # pdb.set_trace()
    if args.cuda >= 0:
        model = model.cuda()
...
def build_model_ada(args, vocab, pretrained_embs, tasks):
    '''
    Build model according to arguments
    '''
    d_word, n_layers_highway = args.d_word, args.n_layers_highway
    ...
    model = MultiTaskModel(args, pair_encoder, _FDS if args.fds else None)
    build_regressor(tasks, model, d_pair)
    build_ada(model, d_pair * 4, dims=args.proj_dims)
    # import pdb
    # pdb.set_trace()
    if args.cuda >= 0:
        model = model.cuda()
...
def build_model_scl(args, vocab, pretrained_embs, tasks):
    '''
    Build model according to arguments
    '''
    d_word, n_layers_highway = args.d_word, args.n_layers_highway
    ...
    # Build model and classifiers
    model = MultiTaskModel(args, pair_encoder, _FDS if args.fds else None)
    build_regressor(tasks, model, int(args.proj_dims/4))
    build_projection(model, d_pair * 4, dims=args.proj_dims)
    # import pdb
    # pdb.set_trace()
    if args.cuda >= 0:
        model = model.cuda()
...
def build_model_naivescl(args, vocab, pretrained_embs, tasks):
    '''
    Build model according to arguments
    '''
    d_word, n_layers_highway = args.d_word, args.n_layers_highway
...
def build_projection(model, d_pair, dims=128):
    '''
    Build the projection
    '''
    model.build_projection(d_pair, dims)
    return


def build_ada(model, in_channel, dims=128):
    '''
    Build the projection
    '''
    model.build_projection_ada(in_channel, dims)
    return


def build_scl(model, d_pair, dims=128):
    '''
    Build the projection
    '''
    model.build_projection(d_pair, dims)
...
class MultiTaskModel(nn.Module):
    def __init__(self, args, pair_encoder, FDS=None):
        ...
        self.reg_pred_layer = nn.Linear(args.d_hid*2, 1)

    def build_regressor(self, task, d_inp):
        layer = nn.Linear(d_inp, 1)
        setattr(self, '%s_pred_layer' % task.name, layer)

    def build_projection(self, d_inp, dims):
        layer = nn.Linear(d_inp, dims)
        setattr(self, 'proj_layer', layer)

    def build_projection_ada(self, d_inp, dims):
        layer = nn.Sequential(nn.Linear(d_inp, d_inp), nn.ReLU(), nn.Linear(d_inp, dims))
        setattr(self, 'proj_layer', layer)

    def forward(self, task=None, epoch=None, input1=None, input2=None, mask1=None, mask2=None, label=None, weight=None, pdf=None):
        pred_layer = getattr(self, '%s_pred_layer' % task.name)
        proj_layer = getattr(self, 'proj_layer')
        ...
        if (self.training and self.FDS is not None) or self.args.regularization_weight>0:
            if self.args.regularization_type in ['scl', 'ada', 'comp3']:
            ...
            elif self.args.regularization_type == 'rank' or self.args.regularization_type == 'conr':
                logits = pred_layer(pair_emb_s)
                out['embs'] = pair_emb_s
                out['labels'] = label
        ...
        elif self.args.loss == 'bmse':
            Loss = BMCLoss(self.args.init_noise_sigma)
            loss = Loss(inputs=logits, targets=label / torch.tensor(5.).cuda(), weights=weight)
        else:
            loss = globals()[f"weighted_{self.args.loss}_loss"](
                inputs=logits, targets=label / torch.tensor(5.).cuda(), weights=weight
            )
...
class HeadlessPairEncoder(Model):
    ...
    def forward(self, s1, s2, m1=None, m2=None):
        ...
        # import pdb
        # pdb.set_trace()
        s1_enc.data.masked_fill_((1 - s1_mask.byte()).bool(), -float('inf'))
        s2_enc.data.masked_fill_((1 - s2_mask.byte()).bool(), -float('inf'))
