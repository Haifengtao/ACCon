...
def build_trainer(args, model, iterator):
    ...
    train_params = Params({'max_vals': args.max_vals, 'cuda_device': args.cuda,
                           ...
                           'lr_decay': .99})
    trainer = SamplingMultiTaskTrainer.from_params(args, model, args.store_dir, iterator, copy.deepcopy(train_params))
    return trainer, train_params, opt_params


class SamplingMultiTaskTrainer():
    def __init__(self, args, model, iterator, patience=2, max_vals=50,
    ...
    def train(self, tasks, validation_interval, train_params, optimizer_params, resume=False):
        ...
        while not should_stop:
            ...
            for batch in itertools.islice(tr_generator, 1):
                ...
                # import pdb
                # pdb.set_trace()
                # print(batch['input1']['words'].size())
                batch['input1']['words'] = batch['input1']['words'].cuda()
                batch['input2']['words'] = batch['input2']['words'].cuda()
                batch['label'] = batch['label'].cuda()
                try:
                    batch['weight'] = batch['weight'].cuda()
                    batch['pdf'] = batch['pdf'].cuda()
                    output_dict = self._forward(batch, task=task, epoch=real_epoch, weight=batch['weight'])
                except:
                ...
                mse_loss_log += output_dict["loss"].data.cpu().numpy()
                # print(loss)
                ...
                if self._args.regularization_weight > 0:
                    ...
                    elif self._args.regularization_type == 'ada' and batch['input1']['words'].size()[0] != 1:
                        Reg_Loss = SupConLoss_admargin()
                        reg_loss = self._args.regularization_weight * Reg_Loss(output_dict['embs'],
                                                                               output_dict['labels'],
                                                                               dist=batch['pdf'])
                        # print(reg_loss)
                        if batch['input1']['words'].size()[0] == self._args.batch_size:
                            loss += reg_loss

                    elif self._args.regularization_type == 'scl' and batch['input1']['words'].size()[0] != 1:
                        Reg_Loss = SupConLoss()
                        reg_loss = self._args.regularization_weight * Reg_Loss(output_dict['embs'], output_dict['labels'])
                        loss += reg_loss
                    elif self._args.regularization_type == 'conr' and batch['input1']['words'].size()[0] != 1:
                        # Reg_Loss = ConR()
                        reg_loss = self._args.regularization_weight * ConR(output_dict['embs'],
                                                                               output_dict['labels'],
                                                                               output_dict['logits']
                                                                               )
                        loss += reg_loss
            ...
            if n_pass // task_info['n_tr_batches'] > real_epoch:
                if self._model.args.fds and real_epoch >= self._model.args.start_update:
                    encodings, labels = [], []
                    with torch.no_grad():
                        for batch in self._iterator(task.train_data, num_epochs=1):
                            batch['input1']['words'] = batch['input1']['words'].cuda()
                            batch['input2']['words'] = batch['input2']['words'].cuda()
                            batch['label'] = batch['label'].cuda()
                            try:
            ...
            if time.time() - task_info['last_log'] > self._log_interval:
                ...
                logging.info("Iter %d (Epoch: %d):reg_loss:%.6f mse_loss:%.6f %s ",
                             n_pass, real_epoch,
                            reg_loss_log / n_batches_since_val,
                             mse_loss_log / n_batches_since_val, description)
                task_info['last_log'] = time.time()
            ...
            if n_pass % (validation_interval) == 0:
                ...
                for task in tasks:
                    ...
                    logging.info("%s: trained on %d batches, %.6f epochs", task.name,
    ...
    def _description_from_metrics(self, metrics):
        return ', '.join([("%s: %.4f" if type(value) is not int else "%s: %d") % (name, value) for name, value in
                          metrics.items()]) + " ||"
    ...
    def _save_checkpoint(self, training_state, best=False):
        ...
        logging.info(
            f"Saved model_state{suffix}.th / training_state{suffix}.th / task_state{suffix}.th / metric_state{suffix}.th "
    ...
    @classmethod
    def from_params(cls, args, model, serialization_dir, iterator, params):
        ...
        return SamplingMultiTaskTrainer(args, model,
                                        iterator,
                                        patience=patience,
                                        max_vals=max_vals,
