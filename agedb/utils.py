...
def prepare_folders(args):
    ...
    if os.path.exists(folders_util[-1]) and not args.resume and not args.pretrained and not args.evaluate:
        print(os.path.exists(folders_util[-1]))
...
def find_nearest(A, vecs):
    ...
    """
    res = np.argmax(np.dot(A, vecs.T) / np.dot(np.linalg.norm(A, axis=1)[:, np.newaxis], np.linalg.norm(vecs, axis=1)[np.newaxis, :]), axis=0)
    return res+1



def adjust_learning_rate(optimizer, epoch, lr, schedules):
    # lr = args.lr
    for milestone in schedules:
        # import pdb
        # pdb.set_trace()
