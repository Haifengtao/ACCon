...
def make_balanced_testset(db="agedb", max_size=30, seed=666, verbose=True, vis=True, save=True):
    ...
    if verbose:
        print(df)
    if save:
        df.to_csv(str(join(BASE_PATH, f"{db}_balanced.csv")), index=False)
    if vis:
...
def make_natural_testset(db="agedb", seed=666, verbose=True, vis=True, save=True):
    file_path = join(BASE_PATH, "meta", f"{db}.csv")
    df = pd.read_csv(file_path)
    df['age'] = df.age.astype(int)
    ...
    import random
    random.seed(seed)
    curr_data = list(df['path'].values)
    random.shuffle(curr_data)
    val_set = curr_data[:2140]
    test_set = curr_data[-2140:]
    ...
    if verbose:
        print(df)
    if save:
        df.to_csv(str(join(BASE_PATH, f"{db}_natural.csv")), index=False)
    if vis:
        _, ax = plt.subplots(3, figsize=(6, 9), sharex='all')
        df_train = df[df['split'] == 'train']
        ax[0].hist(df_train['age'], range(max(df['age'])))
