...
class BMCLoss(_Loss):
    ...
    def forward(self, pred, target, weight):
        ...
        loss = bmc_loss(pred, target, noise_var, weight)
        return loss
...
class SupConLoss_admargin(torch.nn.Module):
    ...
    def forward(self, features, labels=None, mask=None, dist=None, norm_val=0.2, scale_s=150):
        ...
        dist_expand = dist.expand(-1, batch_size)
...
class SupConLoss_comp_v2(torch.nn.Module):
    """Supervised Contrastive Learning: https://arxiv.org/pdf/2004.11362.pdf.
    It also supports the unsupervised contrastive loss in SimCLR"""

    def __init__(self, max_inernal=100, temperature=1, contrast_mode='all',
                 base_temperature=1, tau=0.000001):
        super(SupConLoss_comp_v2, self).__init__()
        self.temperature = temperature
        self.max_inernal = max_inernal
        self.contrast_mode = contrast_mode
        self.base_temperature = base_temperature
...
class SupConLoss_comp_v3(torch.nn.Module):
    """Just for one batch!!!"""

    def __init__(self, max_inernal=100, temperature=1, contrast_mode='all',
                 base_temperature=1):
        super(SupConLoss_comp_v3, self).__init__()
        self.temperature = temperature
        self.max_inernal = max_inernal
        self.contrast_mode = contrast_mode
        self.base_temperature = base_temperature
    ...
    def forward(self, features, labels=None, mask=None, dist=None, norm_val=0.2, scale_s=150):
        ...
        logits_mask = torch.scatter(
            torch.ones_like(mask),
            1,
            torch.arange(batch_size * 1).view(-1, 1),
            0
        )
        # print(logits_mask)
        ...
        mean_log_prob_pos = ((mask * log_prob).sum() + 0.00000001) / (mask.sum() + 0.00000001)
...
class SupConLoss_comp_BCE(torch.nn.Module):
    """Supervised Contrastive Learning: https://arxiv.org/pdf/2004.11362.pdf.
    It also supports the unsupervised contrastive loss in SimCLR"""

    def __init__(self, max_inernal=100, temperature=1, contrast_mode='all',
                 base_temperature=1, tau=0.000001):
        super(SupConLoss_comp_BCE, self).__init__()
        self.temperature = temperature
        self.max_inernal = max_inernal
        self.contrast_mode = contrast_mode
        self.base_temperature = base_temperature
    ...
    def forward(self, features, labels=None, mask=None, dist=None, norm_val=0.2, scale_s=150):
        ...
        contrast_count = features.shape[1]
        views_resamples_feat = []
        views_resamples_label = []
        for i in range(contrast_count):
            batch_unique_targets = torch.unique(labels)
            if len(batch_unique_targets) < len(labels):
                sampled_indices = []
                for target in batch_unique_targets:
                    sampled_indices.append(random.choice((labels == target).nonzero()[:, 0]).item())
                x = features[sampled_indices, i, :]
            ...
            # features = x
            # labels = y
            views_resamples_feat.append(x)
            views_resamples_label.append(y)
        features = torch.stack(views_resamples_feat, dim=1)
        labels = torch.concat(views_resamples_label, dim=0)
        ...
        logits_mask = torch.scatter(
            torch.ones_like(mask),
            1,
            torch.arange(contrast_feature.size(0)).view(-1, 1).cuda(),
            0
        )
...
if __name__ == '__main__':
    ...
    features = torch.rand((16, 2, 24)).cuda()
    labels = torch.randint(0, 3, (16, 1)).cuda()
    # print(labels)
    lossss = SupConLoss_comp_BCE()
